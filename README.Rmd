...
Where $\text{ps}_{i,c,m}$ and  $`\sqrt{3x-1}+(1+x)^2`$  denotes the performance score for site $i$ belonging to class $c$ and metric $m$, $\text{bl}_{c,}$ and $\text{ref}_{c,m}$ represent the metric-specific bottom line and reference values for class $c$ respectively, while $\text{value}_{i,m}$ stands for the measured or modeled value of metric $m$ in site $i$.
